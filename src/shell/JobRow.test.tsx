...
describe('JobRow (#355)', () => {
  ...
  it('done job renders NO .shimmer element', () => {
    const { container } = render(<JobRow job={makeJob({ status: 'done', pct: 100 })} />);
  ...
  it('finished job with onJobDelete shows trash button', () => {
    ...
    render(<JobRow job={makeJob({ status: 'done', pct: 100 })} onJobDelete={onJobDelete} />);
  ...
  it('failed job with onJobDelete shows trash button', () => {
    const onJobDelete = vi.fn();
    render(<JobRow job={makeJob({ status: 'failed', pct: 60 })} onJobDelete={onJobDelete} />);
    expect(screen.getByTestId('job-delete-job-1')).toBeTruthy();
  });

  it('running job does NOT show trash button even with onJobDelete', () => {
    const onJobDelete = vi.fn();
    render(<JobRow job={makeJob({ status: 'running', pct: 30 })} onJobDelete={onJobDelete} />);
