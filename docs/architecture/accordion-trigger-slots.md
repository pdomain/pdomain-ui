...
## Optional slots preserve default trigger behavior
...
the chevron. An undefined chevron uses the built-in glyph. A value of `false`
suppresses it, while a React node replaces it. With no new props, the original
DOM path and styling remain unchanged.

## Consumer migration and keyboard review remain unproven

The component conforms to its design. The repository does not prove that every
consumer removed its raw-Radix wrapper. Interactive trailing content still
requires consumer-level keyboard review.

## Slots remain additive and preserve event behavior
...
## Code, tests, CSS, and history verify the design
