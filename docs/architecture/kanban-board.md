...
selection, staging, and persistence. A move emits the item IDs and the source
and target column IDs. It also reports whether pointer or keyboard input started
the move.

Dragging a selected item moves the selected batch. Dragging an unselected item
moves only that item.
...
sortable coordinate getter and accessible announcements.

The board does not call an API or provide Apply and Discard controls.
